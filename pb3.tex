...
A $2^k$ x  $2^k$ board. We divide and conquer by going into all 4 sub-quadrants, splitting up the board into 4 equally sized sub-quadrants. We continue this until we reach all 2x2 sub-quadrants, from here, we fill the trivial sub-quadrant with a spot missing. Now we can consider this sub-quadrant to be itself a missing spot, and all other recursive calls can back out a step. From here every 2x2 grid can be treated as a 1x1 grid. This means we now have a $2^{k-1}$ x  $2^{k-1}$ grid with a single spot missing. \\
Wrap Up: This proves our hypothesis, since we assumed that a board of size $k-1$ x $k-1$ must follow our hypothesis.
