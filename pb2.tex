...
To get to that range takes $O(log(h))$ time which to trivial to find.\\
The range binary searched will be $(2^{k+1} -2^k)$, so the total runtime would be...\\
$log_2(2^{k+1} -2^k) \leq k + C \rightarrow O(log(h))$.\\\\
...
Induction Step: \\
Assume: \\
$\forall i < h - 1$, it returns the house that has the Party.\\

\hspace*{6mm} Now consider the case where our house is at h. The binary search function considers some range $[i, 2i]$ where $i=\lfloor{log_2(h)}\rfloor$. We also know this will return the correct result as, we know $h >= i$ and $h <= 2i$. Meaning we have not throw out the correct h and do not have to consider any previous numbers. Therefore, our binary search function can find the correct h given the range.\\
