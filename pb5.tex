...
% This psuedo-code is generally pretty messy, may need to change.

Runtime: 
\begin{quote}
	$T(n) = T(n / 4) + cn$\\
	Work done: $n + \frac{n}{4} + \frac{n}{16} + ... + 1 \rightarrow O(n)$
\end{quote}
...
\begin{quote}
	...
	\end{quote}
	Now consider a n x n matrix. We first survey the center rows and columns to check if any of those points are a local minimum. If that is the case, we return the point. \checkmark \\
	Otherwise, we take the smallest point among those surveyed and move to its smallest neighbor. That point must be in one of the quadrants. That's because the original smallest point is the smallest among the rows and columns surveyed.\\
